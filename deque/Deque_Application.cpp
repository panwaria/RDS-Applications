...
Author: Konark Gill
Date: 24-03-09
Description: To apply Full-Retroactivity of Deque
             Data Structure
*/
#include<stdio.h>
#include<iostream.h>
...
if(value==1)
{
	printf("\tEnter the resource no to insert at right end:");
	scanf("%d",&fvalue);
	fflush(stdin);
}
if(count<0)
...
if(value==1)
{
	printf("\tEnter the reource no to insert at left end:");
	scanf("%d",&fvalue);
	fflush(stdin);
}
...
q=p;
	while(1)
	{
		...
		if(((p!=NULL)&&(p->right!=NULL))&&((p->right)->right!=NULL))
		{
			...
			if(r1->time<t)
			{
				printf("\tSufficint no of reources are not present at this time \n");
				break;
			}
		}
		if((p!=NULL)&&(p->right==NULL))
...
int nofrr;
	while(1)
	{
		int ch;
		int value;
		printf("\n\tenter 1 to pushR(x) operation to push resources on the queue\n");
		printf("\tenter 2 to popR(x) operation to pop resources of the queue \n");
...
fflush(stdin);
	if(ch==1)
	{
		...
		if(m>0)
		{
		  printf("\tNo of resources present are = %d and so task can be executed \n",r);
		}
		if(m==0)
		{
		  printf("\tSufficient no of resouces are not present \n");
		}
	}
	if(ch==2)
	{
